...
export default async function Home() {
  ...
  return (
		<main className={styles.development}>
			...
			<section>
				<p>
					{translations("introduction.fields")}
				</p>
				<p>
					{translations.rich("introduction.curricullum", { 
            emoji: (chunks) => <span className="emoji">{chunks}</span>, 
            curriculumLink: (chunks) => <Link href="https://kuzminklk.notion.site/31154d4de03580d7b5ceca5b187846be" target="blank" rel="noopener noreferrer">{chunks}</Link>
          })}
				</p>
				<p>
					{translations("introduction.practice")}
				</p>
				<p>
			...
			<h1><span className={styles.emoji}>💾</span> {translations("projects.header")}</h1>
			<p>
				...
				<div className={styles.grid}>
					{projects.map((project) => {
						const translatedProject:Project = {
							...
							description: translations(`projects.web-interfaces.${project.id}.description`),
							imageAlternative: translations(`projects.web-interfaces.${project.id}.image-alternative`),
