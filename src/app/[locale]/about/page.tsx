  ...
  return (
    <main className={styles.about}>
      <section className={styles.greet}>
        <h1>
          <span className={styles.emoji}>👋</span> {translations("greet.hi")}<br/><em>{translations("greet.introduction")}</em>
        </h1>
        <Image src={Me} alt={translations("greet.photo")} placeholder="blur"></Image>
      </section>
      <section className={styles.about}>
        <p>
          {translations.rich("about.role", { strong: (chunks) => <strong>{chunks}</strong> })}
        </p>
        <p>
          {translations.rich("about.names", { emphasize: (chunks) => <em>{chunks}</em> })}
        </p>
        <p>
        ...
        <p>
          <em>
            {translations.rich("explore", { 
              emoji: (chunks) => <span className="emoji">{chunks}</span>, 
              scroll: (chunks) => <a onClick={scrollToTop}>{chunks}</a> 
            })}
          </em>
        </p>
      </section>
